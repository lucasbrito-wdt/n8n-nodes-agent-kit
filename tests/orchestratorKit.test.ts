...
describe('Orchestrator supervisor pattern', () => {
  it('calls a sub-agent tool and returns the final response', async () => {
    const researcher: SubAgent = {
      ...
      stateless: false, actions: [], outputContentKey: 'content_raw', outputInstructionsKey: 'instructions',
  ...
  it('handles multiple sub-agents', async () => {
    const researcher: SubAgent = { name: 'researcher', description: 'research', stateless: false, actions: [], outputContentKey: 'content_raw', outputInstructionsKey: 'instructions', call: async () => ({ response: 'facts', usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0, iterations: 0 } }) };
    const writer: SubAgent = { name: 'writer', description: 'write', stateless: false, actions: [], outputContentKey: 'content_raw', outputInstructionsKey: 'instructions', call: async () => ({ response: 'article', usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0, iterations: 0 } }) };
  ...
  it('passes context.task to stateless sub-agent', async () => {
    ...
    const agent: SubAgent = {
      ...
      stateless: true, actions: [], outputContentKey: 'content_raw', outputInstructionsKey: 'instructions',
